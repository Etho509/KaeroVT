(function(){
  ...
  const whisper=document.getElementById('whisper');let whisperTimer;
  function showWhisper(message,duration){clearTimeout(whisperTimer);whisper.textContent=message;whisper.classList.add('show');whisperTimer=setTimeout(()=>whisper.classList.remove('show'),duration||6500)}
  ...
  document.querySelectorAll('.footnote-trigger').forEach(function(trigger){trigger.addEventListener('click',function(){showWhisper(this.dataset.secret,6500)})});
  ...
  let titleClicks=0,titleTimer;document.getElementById('title').addEventListener('click',function(){clearTimeout(titleTimer);titleClicks++;titleTimer=setTimeout(()=>titleClicks=0,3000);if(titleClicks>=5){titleClicks=0;document.body.classList.remove('mirrored');void document.body.offsetWidth;document.body.classList.add('mirrored');setTimeout(()=>document.body.classList.remove('mirrored'),2900)}});
  ...
  let keys='';addEventListener('keydown',function(e){keys=(keys+e.key.toLowerCase()).slice(-20);if(keys.endsWith('house'))location.href='login.html';if(keys.endsWith('office'))document.getElementById('footnote-3').textContent='The office door is open now.';if(keys.endsWith('longmancooking'))showWhisper('FILE REJECTED: PROSE QUALITY BELOW ARCHIVAL THRESHOLD',7000)});
  setInterval(function(){if(Math.random()<.28&&!whisper.classList.contains('show'))showWhisper(messages[Math.floor(Math.random()*messages.length)],5000)},11000);
})();
